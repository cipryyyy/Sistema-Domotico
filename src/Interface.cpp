...
void Interface::turnOn(int id) {                        //Accensione manuale
    ...
    if (Cpos != INT_MIN) {
        ...
        std::cout << _cleaner(devicesCP[Cpos].getNome()) + " avviato";
        ...
        std::cout << _cleaner(devicesM[Mpos].getNome()) + " acceso";
...
void Interface::turnOn(int id, int start) {             //Accensione programmata
    ...
    if (Cpos != INT_MIN) {
        ...
        std::cout << "Accensione di " << _cleaner(devicesCP[Cpos].getNome()) + " impostata";
...
void Interface::turnOn(int id, int start, int end) {    //routine
    ...
    if (Cpos != INT_MIN) {
        ...
        //Avvio la routine
        std::cout << "Routine di " << _cleaner(devicesM[Mpos].getNome()) + " impostata";
...
void Interface::turnOff(int id) {                       //spegnimento
    ...
    if (Cpos != INT_MIN) {
        ...
        if (KW - devicesM[Mpos].getConsumo() > maximumKW) throw IllegalGeneratorShutdownException();  //Se cerco di spegnere un generatore e i KW non bastano
...
void Interface::resetTimers() {                         //Resetta tutte le routine
    ...
    std::cout << "Reset dei timer effettuato con successo" << std::endl;
}
...
void Interface::installM(std::string name, double consumo, bool isOn, bool autoTurnOff) {     //Installa device M
	...
	//Creazione ID
    int id;
    if (counterM + counterCP >= maximumDV) throw DeviceLimitException(); // Eccezione, troppi Device installati
    counterM++;
...
void Interface::installCP(std::string name, double consumo, bool isOn, int durataCiclo) {   //Installa device CP
