...
void Interface::turnOn(int id) {                        //Accensione manuale
    ...
    if (Cpos != INT_MIN) {
        ...
        if (KW + devicesCP[Cpos].getConsumo() > maximumKW) {    //Se supero i KW lancio l'eccezione
            log->log(Logger::ERROR, "OverKWException");
        ...
        for (int i : IDreq) {
            if (i == id + maximumDV) {
                log->log(Logger::ERROR, "TimerAlreadySetException");
        ...
        if (devicesM[Mpos].isOn()) {
            log->log(Logger::ERROR, "DeviceAlreadyOnException");
...
void Interface::turnOn(int id, int start, int end) {    //routine
    ...
    if (Cpos != INT_MIN) {
        log->log(Logger::ERROR, "CPIllegalInstructionException");
...
void Interface::turnOff(int id) {                       //spegnimento
    ...
    if (Cpos != INT_MIN) {
        ...
        if (!devicesM[Mpos].isOn()) {
            log->log(Logger::ERROR, "DeviceAlreadyOffException");
        ...
        if (KW - devicesM[Mpos].getConsumo() > maximumKW) {
            log->log(Logger::ERROR, "IllegalGeneratorShutdownException");
...
void Interface::forceOff(int id) {
    ...
    if (Cpos == INT_MIN) {      //Se non è un dispositivo CP
        log->log(Logger::ERROR, "ForceOnMException");
...
void Interface::removeTimer(int id) {                   //Rimuove le routine di un device
    ...
    if (on) {
        ...
        log->log(Logger::INFO, "Timer di ID " + std::to_string(id) + " rimosso dall'ora " + std::to_string(t));
        return;
    }
    log->log(Logger::INFO, "Timer di ID " + std::to_string(id) + " rimosso dall'ora " + std::to_string(t+1));
...
void Interface::setTime(int time) {                     //Scorrimento del tempo
    ...
    log -> log(Logger::INFO, "Orario aggiornato a " + std::to_string(time));
}
...
void Interface::resetTime() {                           //Resetta il tempo
    ...
    log -> log(Logger::INFO, "Orario resettato a 0");
}
...
void Interface::show() {                                //Mostra tutti i device
    ...
    log -> log(Logger::INFO, "Stato dei devices mostrato");
}
void Interface::show(int id) {                          //Mostra un device
    ...
    if (Cpos != INT_MIN) {
        std::cout   << _cleaner(devicesCP[Cpos].getNome())
					...
					<< " ore\n";
        log -> log(Logger::INFO, "Device " + std::to_string(id) + " mostrato");
    } else if (Mpos != INT_MIN) {
...
void Interface::installM(std::string name, double consumo, bool autoTurnOff, bool isOn) {     //Installa device M
    ...
    for (int i = 0; i < counterM; i++) {
        if (NSCcheck(devicesM[i].getNome(), name)) {
            log -> log(Logger::ERROR, "DuplicateDeviceException");
    ...
    if (counterM + counterCP >= maximumDV) {
        log -> log(Logger::ERROR, "DeviceLimitException");
    ...
    log -> log(Logger::INFO, "Device " + name + " installato, ID: " + std::to_string(id) + ", consumo: " + std::to_string(consumo) + "KW, autoTurnOff: " + std::to_string(autoTurnOff) + ", isOn: " + std::to_string(isOn));
}
void Interface::installCP(std::string name, double consumo, int durataCiclo, bool isOn) {   //Installa device CP
    ...
    log -> log(Logger::INFO, "Device " + name + " installato, ID: " + std::to_string(id) + ", consumo: " + std::to_string(consumo) + "KW, durataCiclo: " + std::to_string(durataCiclo) + "h, isOn: " + std::to_string(isOn));
}
    ...
    for (int i = 0; i < counterCP; i++) {			//Controllo se il device è CP
        if (devicesCP[i].getID() == id) {
            ...
            if (devicesCP[pos].isOn()) {
                log -> log(Logger::ERROR, "DeviceIsRunningException");
			...
			counterCP--;
            log -> log(Logger::INFO, "Device " + name + " disinstallato, ID: " + std::to_string(id));
...
int Interface::searchID(std::string name) {			//Ritorna l'ID dato il nome
    log -> log(Logger::INFO, "Ricerca ID per nome" + name);
	for (int i = 0; i < counterCP; i++) {
    ...
    log -> log(Logger::ERROR, "NameNotFoundException");
...
double Interface::debugKWs() {            //Debug KW
    ...
    log -> log(Logger::DEBUG, "KW attuali: " + std::to_string(KW));
    return KW;
}
int Interface::debugTime(bool logging) {              //Debug time
    if (!logging) {
        log -> log(Logger::DEBUG, "Orario attuale: " + std::to_string(t));
        std::cout << "#DT " << t << std::endl;
    }
    return t;
}
void Interface::debugCounters() {         //Debug counters
    log -> log(Logger::DEBUG, "Contatori: CP: " + std::to_string(counterCP) + " M: " + std::to_string(counterM));
    std::cout << "#DC CP: " << counterCP << " M:" << counterM << std::endl;
}
void Interface::debugTOS() {
    log -> log(Logger::DEBUG, "Calcolo TOS");
    ...
    log -> log(Logger::DEBUG, "TOS: ");
    std::cout << "#TOS [";
    for (int i : sequence) {
        log -> log(Logger::DEBUG, std::to_string(i));
        std::cout << i << ", ";
    }
    std::cout << "\b\b]" << std::endl;
    log -> log(Logger::DEBUG, "SIZE = " + std::to_string(sequence.size()));
